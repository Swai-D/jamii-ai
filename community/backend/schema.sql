...
-- posts — image uploads (already exists in original schema, but guide says add it)
-- Original schema: image_url TEXT
-- Adding it safely:
...
ALTER TABLE challenges
  ...
  ADD COLUMN IF NOT EXISTS difficulty_str VARCHAR(50), -- existing is enum, guide uses varchar
...
ALTER TABLE notifications
  ADD COLUMN IF NOT EXISTS type_str     VARCHAR(50) DEFAULT 'general', -- existing type is enum
...
CREATE INDEX IF NOT EXISTS idx_users_full_search ON users USING gin(
  to_tsvector('english',
...
CREATE INDEX IF NOT EXISTS idx_resources_full_search ON resources USING gin(
  to_tsvector('english',
    coalesce(title,'') || ' ' || coalesce(description,'') || ' ' || coalesce(tags::text,'')
  )
);

CREATE INDEX IF NOT EXISTS idx_jobs_full_search ON jobs USING gin(
  to_tsvector('english',
