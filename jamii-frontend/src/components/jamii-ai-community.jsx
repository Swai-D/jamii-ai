...
const INITIAL_POSTS = [
  ...
  { id: 2, author: "Jonas Kimaro", handle: "jonaskimaro", avatar: "JK", color: "#F87171", role: "AI Architect · Dodoma", time: "Saa 1 iliyopita", category: "mradi", tag: "Mradi", content: "🚀 Nimekamilisha mradi wangu wa kwanza wa RAG system kwa Kiswahili! Bot enaweza kujibu maswali kutoka documents za Kiswahili kwa usahihi wa 89%. Stack: LangChain + Claude API + Pinecone. Code iko GitHub — link kwenye comments. Shukrani jamii! 🇹🇿", likes: 87, comments: 15, bookmarks: 32, liked: true, bookmarked: false, commentList: [{ author: "Amina H.", color: "#4ECDC4", text: "Hongera sana Jonas!! Hii ni milestone kubwa." }, { author: "Davy M.", color: "#F5A623", text: "This is exactly what Tanzania needs. Nzuri sana bro!" }] },
...
function PostCard({ post, onLike, onBookmark, onComment, me }) {
      ...
      {showComments && (
        <div style={{ marginTop: 18, paddingLeft: 56, borderTop: "1px solid rgba(255,255,255,0.05)", paddingTop: 16 }}>
          ...
          <div style={{ display: "flex", gap: 8, marginTop: 6 }}>
            ...
            <input value={newComment} onChange={e => setNewComment(e.target.value)} onKeyDown={e => { if (e.key === "Enter" && newComment.trim()) { onComment(post.id, newComment); setNewComment(""); } }} placeholder="Andika jibu..." style={{ flex: 1, background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 8, padding: "8px 12px", color: "#DCE6F0", fontSize: 13, fontFamily: "'Syne',sans-serif", outline: "none" }} />
            <button onClick={() => { if (newComment.trim()) { onComment(post.id, newComment); setNewComment(""); } }} style={{ background: "#F5A623", color: "#0A0F1C", border: "none", borderRadius: 8, padding: "8px 14px", cursor: "pointer", fontFamily: "'Syne',sans-serif", fontWeight: 800, fontSize: 13 }}>↑</button>
          </div>
        </div>
      )}
    </div>
        ...
        {devs.map(dev => (
          <div key={dev.name} style={{ background: "rgba(255,255,255,0.025)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 14, padding: 18, transition: "all 0.2s", cursor: "pointer" }}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              ...
              <button style={{ background: dev.available ? "#F5A623" : "rgba(255,255,255,0.05)", color: dev.available ? "#0A0F1C" : "rgba(220,230,240,0.2)", border: "none", padding: "6px 14px", borderRadius: 7, cursor: dev.available ? "pointer" : "default", fontFamily: "'Syne',sans-serif", fontWeight: 800, fontSize: 11 }}>{dev.available ? "Hire →" : "Busy"}</button>
            </div>
          </div>
        ))}
      </div>
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", gap: 5, marginBottom: 20, flexWrap: "wrap" }}>
        {sectors.map(s => <button key={s} onClick={() => setFilter(s)} style={{ padding: "5px 13px", borderRadius: 20, fontSize: 11, fontWeight: 700, cursor: "pointer", border: `1px solid ${filter === s ? "#F5A623" : "rgba(255,255,255,0.1)"}`, background: filter === s ? "rgba(245,166,35,0.12)" : "transparent", color: filter === s ? "#F5A623" : "rgba(220,230,240,0.4)", fontFamily: "'Space Mono',monospace", transition: "all 0.2s" }}>{s}</button>)}
      </div>
...
function VyuoPage() {
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", gap: 5, marginBottom: 20, flexWrap: "wrap" }}>
        {types.map(t => <button key={t} onClick={() => setFilter(t)} style={{ padding: "5px 13px", borderRadius: 20, fontSize: 11, fontWeight: 700, cursor: "pointer", border: `1px solid ${filter === t ? "#60A5FA" : "rgba(255,255,255,0.1)"}`, background: filter === t ? "rgba(96,165,250,0.12)" : "transparent", color: filter === t ? "#60A5FA" : "rgba(220,230,240,0.4)", fontFamily: "'Space Mono',monospace", transition: "all 0.2s" }}>{t}</button>)}
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
        {filtered.map(inst => (
...
function ChangamotoPage() {
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", gap: 0, marginBottom: 24, background: "rgba(245,166,35,0.06)", border: "1px solid rgba(245,166,35,0.14)", borderRadius: 12, overflow: "hidden" }}>
                ...
                {selected?.id === ch.id && (
                  <div style={{ marginTop: 16, paddingTop: 16, borderTop: "1px solid rgba(255,255,255,0.07)" }}>
                    ...
                    <div style={{ display: "flex", gap: 8 }}>
                      {ch.status === "open" && <button style={{ background: "#F5A623", color: "#0A0F1C", border: "none", padding: "10px 22px", borderRadius: 9, cursor: "pointer", fontFamily: "'Syne',sans-serif", fontWeight: 800, fontSize: 13 }}>Jiandikishe →</button>}
                      <button style={{ background: "transparent", color: "rgba(220,230,240,0.55)", border: "1px solid rgba(255,255,255,0.1)", padding: "10px 22px", borderRadius: 9, cursor: "pointer", fontFamily: "'Syne',sans-serif", fontWeight: 700, fontSize: 13 }}>Maelezo</button>
                    </div>
                  </div>
                )}
              </div>
...
function RasilimaliPage() {
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", gap: 5, marginBottom: 20, flexWrap: "wrap" }}>
        {types.map(t => <button key={t} onClick={() => setFilter(t)} style={{ padding: "5px 13px", borderRadius: 20, fontSize: 11, fontWeight: 700, cursor: "pointer", border: `1px solid ${filter === t ? "#34D399" : "rgba(255,255,255,0.1)"}`, background: filter === t ? "rgba(52,211,153,0.12)" : "transparent", color: filter === t ? "#34D399" : "rgba(220,230,240,0.4)", fontFamily: "'Space Mono',monospace", transition: "all 0.2s" }}>{t}</button>)}
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
        {filtered.map(r => {
          ...
          return (
            <div key={r.id} style={{ background: "rgba(255,255,255,0.025)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 14, padding: 18, display: "flex", flexDirection: "column", cursor: "pointer", transition: "all 0.2s" }}>
              ...
              <button style={{ background: tc, color: "#0A0F1C", border: "none", padding: "8px", borderRadius: 8, cursor: "pointer", fontFamily: "'Syne',sans-serif", fontWeight: 800, fontSize: 12, marginTop: "auto" }}>Pakua / Angalia →</button>
            </div>
          );
        })}
      </div>
...
export default function JamiiAICommunity({ user, onLogout }) {
  const ME = { 
    name: user?.name || "Davy Mwangi", 
    handle: user?.handle || "davyswai", 
    avatar: user?.name ? user.name.split(" ").map(w => w[0]).join("") : "DM", 
    color: "#F5A623", 
    role: user?.role || "AI Developer", 
    loc: user?.city || "Dar es Salaam" 
  };
  ...
  return (
    <div style={{ fontFamily: "'Syne',sans-serif", background: "#0A0F1C", color: "#DCE6F0", minHeight: "100vh", display: "flex", justifyContent: "center" }}>
      <style>{`
        ...
        * { margin:0; padding:0; box-sizing:border-box; }
        ...
        .ftab:hover{background:rgba(255,255,255,0.05)}
        ...
        /* Media query kwa ajili ya responsiveness */
        @media (max-width: 1100px) {
          .right-sidebar { display: none !important; }
        }
        @media (max-width: 768px) {
          .left-sidebar { width: 80px !important; }
          .left-sidebar span:not(.icon) { display: none !important; }
          .left-sidebar .nav-item { justify-content: center; padding: 12px !important; }
        }
      `}</style>

      <div style={{ display: "flex", width: "100%", maxWidth: "1250px", position: "relative" }}>
        {notification && <div style={{ position: "fixed", bottom: 28, right: 28, zIndex: 999, background: "#F5A623", color: "#0A0F1C", padding: "11px 18px", borderRadius: 9, fontWeight: 800, fontSize: 13, fontFamily: "'Space Mono',monospace", animation: "notif 2.5s ease forwards", pointerEvents: "none" }}>{notification}</div>}

        {/* ── LEFT SIDEBAR ── */}
        <aside className="left-sidebar" style={{ width: 240, flexShrink: 0, height: "100vh", position: "sticky", top: 0, borderRight: "1px solid rgba(255,255,255,0.06)", padding: "20px 12px", display: "flex", flexDirection: "column", gap: 2, overflowY: "auto" }}>
          ...
          {NAV_ITEMS.map(item => (
            <div key={item.id} className={`nav-item ${activeNav === item.id ? "active" : ""}`} onClick={() => setActiveNav(item.id)}>
              <span className="icon" style={{ fontSize: 13, fontFamily: "'Space Mono',monospace", opacity: activeNav === item.id ? 1 : 0.4 }}>{item.icon}</span>
              ...
              {item.badge && <span style={{ background: "#F5A623", color: "#0A0F1C", fontSize: 9, fontWeight: 800, padding: "2px 6px", borderRadius: 9, fontFamily: "'Space Mono',monospace" }}>{item.badge}</span>}
            </div>
          ))}
          <div style={{ flex: 1 }} />
          <div style={{ borderTop: "1px solid rgba(255,255,255,0.06)", paddingTop: 14 }}>
            <div onClick={onLogout} style={{ marginBottom: 12, paddingLeft: 6, fontSize: 11, cursor: "pointer", color: "rgba(220,230,240,0.38)", fontFamily: "'Space Mono',monospace" }}>← Toka (Logout)</div>
            <div style={{ display: "flex", gap: 10, alignItems: "center", padding: "9px 6px", borderRadius: 9, cursor: "pointer" }}>
              ...
              <div style={{ overflow: "hidden" }}>
                <div style={{ fontWeight: 700, fontSize: 12, whiteSpace: "nowrap", textOverflow: "ellipsis" }}>{ME.name}</div>
        ...
        {/* ── MAIN ── */}
        <main style={{ flex: 1, minWidth: 0, borderRight: "1px solid rgba(255,255,255,0.06)", overflowY: "auto", height: "100vh" }}>
          ...
          <div style={{ padding: "18px 22px", maxWidth: "700px", margin: "0 auto" }}>
            ...
            {(activeNav === "nyumbani" || activeNav === "gundua") && (
              <>
                ...
                {filteredPosts.map(post => <PostCard key={post.id} post={post} onLike={handleLike} onBookmark={handleBookmark} onComment={handleComment} me={ME} />)}
              </>
            )}
        ...
        {/* ── RIGHT SIDEBAR ── */}
        <aside className="right-sidebar" style={{ width: 310, flexShrink: 0, height: "100vh", overflowY: "auto", padding: "20px 16px", display: "flex", flexDirection: "column", gap: 18 }}>
        {/* Search */}
        <div style={{ position: "relative" }}>
          <input placeholder="Tafuta JamiiAI..." style={{ width: "100%", background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 10, padding: "10px 14px 10px 34px", color: "#DCE6F0", fontFamily: "'Syne',sans-serif", fontSize: 13 }} />
